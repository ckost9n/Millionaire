...
class MainViewController: UIViewController {
    ...
    private func nextQuestion(qNbr: Int, tag: Int) {
        if qNbr == 9 {
            ...
            let attributedString = NSMutableAttributedString(string: longString, attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 15)])
            attributedString.setAttributes([NSAttributedString.Key.font : UIFont.boldSystemFont(ofSize: 26), NSAttributedString.Key.foregroundColor : UIColor(red: 4/255, green: 213/255, blue: 134/255, alpha: 1)], range: longestWordRange)
    ...
    private func showIncorrectAlert(tag: Int) {
        ...
        let gameSession = GameSession(questionsTotal: "Вопрос " + "\(qNbr + 1)" + " из 10", moneyTotal: "Выигрыш: " + winArray[qNbr] + "₴", date: Date())
        
        Game.shared.addGameSession(gameSession: gameSession)
